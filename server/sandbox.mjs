...
export function buildSandboxData(db) {
  ...
  let tenants = [{ id: "medyatra", name: "MedYatra (own acquisition)", mode: "own" }];
  try {
...
<div class="topbar">
  <div class="logo">MedYatra <small>· patient-journey studio</small></div>
  <div class="spacer"></div>
  <div class="tsel">Viewing as
    <select id="tenant" aria-label="brand"></select>
    <span class="wl-note" id="wl-note">MedYatra’s own acquisition brand.</span>
  </div>
...
<div id="app">
  ...
  <main class="stage-main">
    <div class="phone">
      <div class="wa-top"><div class="wa-av">M</div><div><b id="wa-brand">MedYatra</b><div class="on">online · typical reply in minutes</div></div></div>
      <div id="thread"></div>
    </div>
...
<title>MedYatra — Patient Journey Sandbox</title><style>${CSS}</style></head>
<body>${bodyHtml()}
...
export function renderSandboxArtifact(db) {
  const data = buildSandboxData(db);
  return `<title>MedYatra — Patient Journey Sandbox</title>
<style>${CSS}</style>
${bodyHtml()}
