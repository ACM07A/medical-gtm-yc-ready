...
export const publishers = {
  ...
  reddit: {
    ...
    async publish({ title, text }, { confirm = false } = {}) {
      ...
      const r = await fetch("https://oauth.reddit.com/api/submit", { method: "POST", headers: { Authorization: `Bearer ${tok}`, "Content-Type": "application/x-www-form-urlencoded", "User-Agent": "MedYatra/1.0" }, body: form });
  ...
  whatsapp: {
    ...
    async publish({ to, text }, { confirm = false } = {}) {
      ...
      if (!(confirm && process.env.POST_LIVE === "1")) return { dryRun: true, would: `WhatsApp message to ${to || "<opt-in list>"}` };
