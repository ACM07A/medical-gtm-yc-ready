...
// no third-party API into the government portal (indianvisaonline.gov.in), and MedYatra does NOT submit on
// the patient's behalf. Since 1 Apr 2025 the e-Medical / e-Ayush visa REQUIRES a system-generated Medical
// Invitation Letter (Appendix II) issued by the registered hospital. So MedYatra's role is deliberately
// narrow: orchestrate the hospital's letter (a HOSPITAL handoff) and hand the patient a country-correct
// checklist. The PATIENT applies themselves and books their own tickets. A VFS concierge can be keyed for a
// tenant later — but the default posture keeps the coordination layer light.
//
...
export function visaSteps() {
  return [
    ...
    { key: "documents", label: "Patient assembles checklist documents (MedYatra supplies the letter + checklist)", owner: "patient", status: "collecting" },
