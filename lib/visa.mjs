// VISA ASSISTANCE (India e-Medical Visa) — a WORKFLOW, not an API. There is no third-party API into the
// government portal (indianvisaonline.gov.in). Since 1 Apr 2025 the e-Medical / e-Ayush visa REQUIRES a
// system-generated Medical Invitation Letter (Appendix II) issued by the registered hospital and emailed to
// the mission. So MedYatra's value is orchestration: trigger the hospital's letter (a HOSPITAL handoff),
// hand the patient a country-correct checklist, guide the portal application, and track status — plus the
// accompanying attendant visas. An optional concierge provider (VFS Global / aggregator) can be keyed later.
//
...
export function visaSteps() {
  return [
    ...
    { key: "documents", label: "Patient assembles checklist documents", owner: "patient (MedYatra guides)", status: "collecting" },
    { key: "apply", label: "Apply on indianvisaonline.gov.in (e-Medical + MED-X for attendants)", owner: "patient (MedYatra guides)", status: "applied" },
    ...
    { key: "frro", label: "FRRO registration if stay > 180 days (within 14 days of arrival)", owner: "patient (MedYatra reminds)", status: "post_arrival" },
  ];
}
