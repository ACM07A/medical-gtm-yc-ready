...
export function welcomeHtml({ treatment, market } = {}) {
  return shell(`
    ...
    <div style="margin-top:34px;display:flex;flex-direction:column;gap:22px">
      ${["Accredited hospitals & named specialists", "Transparent package pricing — no surprises", "A coordinator with you the whole way"].map((t) => `
    ...
    <div style="margin-top:26px;font-size:30px;color:${BRAND.blue};font-weight:700">${treatment ? `Your ${treatment} options, shared personally.` : "Your care journey, guided end to end."}</div>
  `);
}
...
export function howItWorksHtml() {
  const steps = [["1", "Share", "Send your reports — we review, no cost"], ["2", "Plan", "Accredited hospital + doctor options & a clear quote"],
    ["3", "Travel", "Visa invitation, flights, stay — all arranged"], ["4", "Care", "Treatment + an interpreter & coordinator with you"]];
  return shell(`
