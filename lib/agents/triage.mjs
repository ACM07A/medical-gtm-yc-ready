...
export async function triage(patientText, { category_hint = null } = {}) {
  ...
  // never wait on an LLM round-trip. The person on the other end may be terrified, so this path does not
  // just flag an internal action — it also carries a calm, human, patient-FACING reply. It is a fixed,
  // pre-reviewed constant (never model-generated on this path), and it stays strictly inside scope: it
  // directs them to emergency services and reassures them a person is coming, without a diagnosis, a
  // severity call, or any clinical instruction — that line holds even, especially, in a crisis.
  if (RED_FLAGS.test(patientText)) {
    return { urgency: "possible_emergency",
      action: "ESCALATE — do not continue the funnel; direct to local emergency services and notify a human immediately",
      patient_message: "Please stop and call your local emergency number now, or go straight to the nearest " +
        "emergency department — your safety matters more than anything else right now. We're holding everything " +
        "on our side, and a member of our team is being alerted to reach you. You are not on your own with this.",
