...
export function optInTemplate({ name, patientFirstName = "your family member" }) {
  return `Hello${name ? " " + name : ""}, this is MedYatra. ${patientFirstName} listed you as the contact ` +
    `to receive updates while they're being treated in India. Reply YES if you'd like a short daily update, ` +
    `or STOP if you'd rather not be contacted. We'll only message you with logistics updates — never anything ` +
    `you haven't agreed to receive.`;
}
