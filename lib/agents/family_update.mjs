...
const SYSTEM = `You write short, warm, honest daily updates to a family member waiting at home while their
relative is being treated abroad. You report LOGISTICAL STATUS ONLY — where they are, that they are being
cared for, what happens next, when the next update will come. You NEVER report a clinical outcome, NEVER say
how the procedure "went," NEVER offer reassurance about medical results ("all went well", "nothing to worry
about") — that is the hospital's to say, not ours, and saying it is not something we are allowed to do. If
you don't know something, say the hospital's team will share it directly. Two to four sentences. Plain,
calm, human — never corporate, never a template with blanks showing.`;
...
function deterministicUpdate(stageLabel, patientFirstName = "your relative") {
  return `Update: ${patientFirstName} ${stageLabel}. We'll message again tomorrow with the next update. ` +
    `If the hospital's team has anything specific to share about the procedure itself, they'll be in touch with you directly.`;
}
