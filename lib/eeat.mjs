...
//
// THE HONEST RESOLUTION of a real tension: MedYatra is a facilitator, so it cannot claim clinical
...
export function jsonLd({ title, description, url, author, reviewedAt, publisher = "MedYatra", citations = [] }) {
  const ld = {
...
export function trustBlock({ author = "MedYatra editorial", reviewedAt, sources = [] } = {}) {
  ...
  return `\n\n---\n\n### About this page\n\n` +
    ...
    `**What this page is not.** MedYatra is a facilitator, not a healthcare provider. Nothing here is medical ` +
