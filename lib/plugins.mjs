...
export function plugins() {
  const list = [
    { id: "text-gen", name: "Text generation (Tier-2 failover)", category: "generation",
      purpose: "Cornerstone pages, proposals, social copy (GLM-5.2 → llama)", envKeys: ["NVIDIA_API_KEY"],
      ready: has("NVIDIA_API_KEY"), requirements: "NVIDIA NIM key (free tier)" },
    { id: "image-gen", name: "Image generation", category: "generation",
      purpose: "Render Instagram carousel image briefs into real images", envKeys: ["(free) or GEMINI_API_KEY"],
      ready: image.available(), requirements: `current: ${image.providerName()}; Gemini 'Nano Banana' for premium (needs billing)` },
