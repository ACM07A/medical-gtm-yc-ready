...
import { stayProvider } from "./stay.mjs";
...
export function plugins() {
  const list = [
    ...
    { id: "visa", name: "Visa assistance (India e-Medical)", category: "services",
      purpose: "Orchestrate the hospital invitation letter + document checklist + attendant visas (a workflow — no government API exists)", envKeys: ["(workflow) or VFS_API_KEY"],
      ready: true, requirements: `self-guided workflow${has("VFS_API_KEY") ? " + VFS concierge" : "; add VFS_API_KEY for concierge"}` },
    { id: "accommodation", name: "Accommodation (near-hospital stay)", category: "services",
      purpose: "Near-hospital, extended-stay, family rooms for patient + relatives, pre/post-op", envKeys: ["BOOKING_AFFILIATE_ID + BOOKING_API_TOKEN / HOTELBEDS_API_KEY / ETG_API_KEY"],
      ready: true, requirements: `${stayProvider()} inventory${stayProvider() === "curated" ? " (curated fallback — key a provider for live rates)" : ""}` },
  ];
