...
//
//   IMAGE_PROVIDER=gemini     GEMINI_API_KEY=...     (Gemini 2.5 Flash Image / "Nano Banana" — top quality)
//   IMAGE_PROVIDER=together   TOGETHER_API_KEY=...   (FLUX.1 schnell/dev/pro — SOTA; free tier: FLUX.1-schnell-Free)
...
//   (fallback)                Pollinations — free, no key (LOW quality; best-effort)
import { writeFileSync, mkdirSync } from "node:fs";
import { dirname } from "node:path";

const KEYS = { gemini: "GEMINI_API_KEY", together: "TOGETHER_API_KEY", openai: "OPENAI_API_KEY", stability: "STABILITY_API_KEY", nvidia: "NVIDIA_API_KEY" };
function provider() {
  if (process.env.IMAGE_PROVIDER) return process.env.IMAGE_PROVIDER;
  if (process.env.GEMINI_API_KEY) return "gemini";        // Nano Banana — top quality when present
  if (process.env.TOGETHER_API_KEY) return "together";    // FLUX — SOTA
  if (process.env.OPENAI_API_KEY) return "openai";
  if (process.env.STABILITY_API_KEY) return "stability";
  return "pollinations";                                  // free fallback (low quality)
}
