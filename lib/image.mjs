...
// Write image bytes with the CORRECT extension sniffed from magic bytes (many providers return JPEG even
// when asked for PNG), so GitHub/viewers render it. Returns the actual path written.
function saveImg(outPath, buf, providerLabel) {
  ...
  return { path: finalPath, provider: providerLabel };
}
...
export async function generate(prompt, outPath, { size = "1024x1024" } = {}) {
  ...
  if (PROVIDER === "cloudflare") {
    ...
    return saveImg(outPath, Buffer.from(b, "base64"), "cloudflare/flux-schnell");
  }
  if (PROVIDER === "huggingface") {
    ...
    return saveImg(outPath, buf, `huggingface/${model.split("/").pop()}`);
  }
  ...
  return saveImg(outPath, Buffer.from(b64, "base64"), PROVIDER);
}
