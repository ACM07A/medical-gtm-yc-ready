...
// the lib/plugins.mjs pattern; hosting access to be provided for the live product). MedYatra's own reading
...
export function relayRecord(v, { recordId, purpose, actor = "system" }) {
  ...
  if (!RELAY_PURPOSES[purpose]) {
    ...
    return { refused: true, reason: `'${purpose}' is not a permitted purpose. Clinical payloads decrypt only for: ${Object.keys(RELAY_PURPOSES).join(" | ")}. MedYatra's own read surface is the envelope.` };
  }
